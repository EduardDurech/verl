...
VALID_CONFIG_TYPE = {"llama", "qwen2", "qwen2_vl", "qwen2_5_vl", "qwen3", "qwen3_moe", "deepseek_v3", "swissai"}
...
class FlopsCounter:
    ...
    def __init__(self, config: PretrainedConfig):
        ...
        self.estimate_func = {
            ...
            "swissai": self._estimate_swissai_flops,
        }
        self.config = config
    ...
    def _estimate_swissai_flops(self, tokens_sum, batch_seqlens, delta_time):
        hidden_size = self.config.hidden_size
        vocab_size = self.config.vocab_size
        num_hidden_layers = self.config.num_hidden_layers
        num_key_value_heads = self.config.num_key_value_heads
        ...
        # SwissAI MLP with XIELU activation uses only 2 linear layers (up_proj, down_proj)
        # No gate_proj for XIELU, unlike SwiGLU which has 3 layers
        mlp_N = hidden_size * intermediate_size * 2
        attn_linear_N = hidden_size * (q_size + k_size + v_size + num_attention_heads * head_dim)

        # SwissAIConfig has qk_norm defaulting to True.
